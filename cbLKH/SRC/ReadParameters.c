...
void ReadParameters()
{
    ...
    while ((Line = ReadLine(ParameterFile))) {
        ...
        } else if (!strcmp(Keyword, "STOP_AT_MAX_COST")) {
            if (!ReadYesOrNo(&StopAtMaxCost))
                eprintf("STOP_AT_MAX_COST: YES or NO expected");
        ...
        } else if (!strcmp(Keyword, "MAX_COST")) {
			if (!(Token = strtok(0, Delimiters)) ||
                !sscanf(Token, GainInputFormat, &MaxCost))
                eprintf("MAX_COST: integer expected");
        } else
